...
const Jobs = () => {
    ...
    return (
        <div className={styles.jobsContainer}>
            ...
            {isModalOpen && <Modal isOpen={isModalOpen} closeFunction={setModalOpen} jobInfo={jobSelected} updateJobsFunction={updateJobItem} statusSuggestions={[...new Set(jobSelected.Statuses.map(job => job.name))]} categories={[...new Set(myInitialJobs.map(job => job.Category))]} statuses={[...new Set(jobSelected.Statuses.map(stat => stat))]} jobtypes={[...new Set(myInitialJobs.map(job => job.Type))]}></Modal>}
